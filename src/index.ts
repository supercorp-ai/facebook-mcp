...
async function main() {
  ...
  if (config.transport === 'http') {
        ...
        try {
          ...
          transport.onclose = async () => {
            ...
            if (sid && sessions.has(sid)) {
              sessions.delete(sid);
              console.log(`[${sid}] Transport closed; removed from session map`);
            }
            try {
              await server.close();
            } catch {
    ...
    app.listen(config.port, () => {
      console.log(`Listening on port ${config.port} (http:${httpMode}) [storage=${config.storage}]`);
    });
  ...
  app.listen(config.port, () => {
    ...
    console.log(`Listening on port ${config.port} (sse) [storage=${config.storage}] using ${headerInfo}`);
  });
}
